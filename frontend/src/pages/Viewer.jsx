...
import { Pencil, MessageCircle, ExternalLink, Trash2, RefreshCw } from "lucide-react";
...
export default function Viewer() {
  ...
  const doDelete = async () => {
    if (!window.confirm(`Delete "${title}" and all its extracted data?`)) return;
  ...
  const pill = (active) =>
    `rounded-md border px-2.5 py-1 font-mono text-[12px] transition-colors ${active ? "border-coal bg-coal text-white" : "border-seam bg-white text-stone-600 hover:border-coal hover:text-coal"}`;
  ...
  return (
    <div>
      <Rise>
        <div className="flex flex-wrap items-start justify-between gap-3">
          ...
          <div className="relative flex flex-wrap items-center gap-2">
            {renaming ? (
              <form onSubmit={doRename} className="absolute right-0 top-9 z-10 flex w-72 gap-2 rounded-xl border border-seam bg-white p-3 shadow-lift">
            ...
            <button onClick={() => { setRenaming((r) => !r); setName(title); }} className={chip}>
            ...
            <button onClick={doDelete} className="flex items-center gap-1.5 rounded-lg border border-seam px-2.5 py-1.5 text-[12.5px] text-stone-500 transition-colors hover:border-red-300 hover:bg-red-50 hover:text-red-700">
      ...
      <Rise delay={0.12}>
        <details className="mt-4 rounded-xl border border-seam bg-white shadow-card">
          <summary className="cursor-pointer select-none list-none px-5 py-3.5 text-[14px] font-semibold">Metadata &amp; Provenance</summary>
      ...
      {isPdf && pages.length > 0 && (
        <Rise delay={0.14}>
          <div className="mt-6 flex flex-wrap items-center gap-1.5">
            ...
            {pages.map((p) => (
              <button key={p.page_no} onClick={() => setParams({ page: String(p.page_no) })}
                      className={pill(activePage === p.page_no)}
      ...
      {isSheet && sheets.length > 0 && (
        <Rise delay={0.14}>
          <div className="mt-6 flex flex-wrap items-center gap-1.5">
            ...
            {sheets.map((s) => (
              <button key={s.sheet_no} onClick={() => setParams({ sheet: String(s.sheet_no) })}
                      className={pill(sheet === s.sheet_no)}>{s.sheet_no}</button>
            ))}
          </div>
        </Rise>
      )}
