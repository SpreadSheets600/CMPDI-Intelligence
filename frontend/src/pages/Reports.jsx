...
export default function Reports() {
  ...
  return (
    <div>
      ...
      <Rise delay={0.05}>
        <div className="mt-6 rounded-xl border border-coalline bg-coalsoft/50 p-5 shadow-card">
          ...
          <form onSubmit={parliamentary} className="mt-3 flex flex-wrap items-start gap-3">
            ...
            <button disabled={busy}
                    className="rounded-lg bg-coal px-5 py-2.5 text-sm font-semibold text-white shadow-card transition-all hover:-translate-y-px hover:bg-amber-500 disabled:opacity-50">
              Generate Draft
            </button>
          </form>
        </div>
        ...
        <div className="mt-5 rounded-xl border border-seam bg-white p-5 shadow-card">
          ...
          <form onSubmit={generate} className="mt-3 flex flex-wrap items-center gap-3">
            ...
            <button type="submit" disabled={busy}
                    className="rounded-lg bg-coal px-5 py-2.5 text-sm font-semibold text-white shadow-card transition-all hover:bg-amber-500 active:translate-y-px disabled:opacity-50">Generate</button>
          </form>
        </div>
      </Rise>
      ...
      <div className="mt-4 grid grid-cols-1 gap-4 md:grid-cols-2">
        {data.reports.length === 0 ? (
          ...
          <article key={r.id} className="flex flex-col rounded-xl border border-seam bg-white p-5 shadow-card transition-all duration-300 hover:-translate-y-0.5 hover:shadow-lift">
            <div className="flex items-start justify-between gap-2">
              <div className="min-w-0">
                <p className="font-semibold">{templateLabel(r.template)}</p>
