...
export default function Compare() {
  ...
  return (
    <div>
        ...
        <div className="mt-6 rounded-xl border border-seam bg-white p-4 shadow-card">
          ...
          <div className="mt-3 flex flex-wrap items-end gap-3">
            ...
            <button type="button" onClick={run} disabled={busy}
                    className="flex items-center gap-2 rounded-lg bg-coal px-5 py-2.5 text-sm font-semibold text-white shadow-card transition-all hover:-translate-y-px hover:bg-amber-500 disabled:opacity-50">
              Compare
            </button>
          </div>
        </div>
      ...
      <div className="mt-6">
        {err && <p className="text-[13.5px] text-red-700">{err}</p>}
        {busy && <p className="animate-pulse text-[13.5px] text-stone-500">Comparing...</p>}
        {result && !result.error && (
          <Rise>
            ...
            {result.fact_changes.length > 0 && (
              <><h2 className="mt-8 text-lg font-semibold">Changed Numerical Facts</h2>
              <div className="mt-3 space-y-1.5">{result.fact_changes.map(changeRow)}</div></>
            )}
            {result.added_facts.length > 0 && (
              <><h2 className="mt-8 text-lg font-semibold">Facts Only in Document B</h2>
              <div className="mt-3 space-y-1.5">{result.added_facts.map((f) => listRow(f, "new"))}</div></>
            )}
            {result.removed_facts.length > 0 && (
              <><h2 className="mt-8 text-lg font-semibold">Facts Only in Document A</h2>
              <div className="mt-3 space-y-1.5">{result.removed_facts.map((f) => listRow(f, "old"))}</div></>
            )}
            {result.new_sections.length > 0 && (
              <><h2 className="mt-8 text-lg font-semibold">New Sections</h2>
              <div className="mt-3 space-y-1.5">{result.new_sections.map(sectionRow)}</div></>
            )}
            {result.removed_sections.length > 0 && (
              <><h2 className="mt-8 text-lg font-semibold">Removed Sections</h2>
              <div className="mt-3 space-y-1.5">{result.removed_sections.map(sectionRow)}</div></>
            )}
