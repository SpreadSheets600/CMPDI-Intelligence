  ...
  useEffect(() => {
    ...
    const onMove = (e) => {
      ...
      if (n !== hovered.current) {
        ...
        // live drag position update happens in tick via pointer offset
      }
    };
    const onDrag = (e) => {
...
export default function Graph() {
  ...
  const closePanel = () => setPicked(null);
      ...
      <Rise delay={0.05}>
        <div className="mt-6 grid gap-5 lg:grid-cols-[1fr_280px]">
          <div className="relative overflow-hidden rounded-xl border border-seam bg-white shadow-card">
            ...
            {picked && (
              <div className="absolute inset-y-0 right-0 w-72 border-l border-seam bg-white/95 p-4 backdrop-blur">
                <div className="flex items-start justify-between gap-2">
                  <h3 className="text-[13px] font-semibold leading-snug">
                    {picked.type === "tag" ? `#${picked.label}` : picked.label}
                  </h3>
                  <button onClick={closePanel} className="text-stone-400 hover:text-ink">✕</button>
                </div>
                {picked.type === "document" && (
                  <>
                    <p className="mt-2 font-mono text-[11px] uppercase text-stone-400">{picked.group}</p>
                    <Link to={`/doc/${picked.ref}`} className="mt-4 block rounded-lg bg-coal px-3 py-2 text-center text-[13px] font-semibold text-white">Open Document</Link>
