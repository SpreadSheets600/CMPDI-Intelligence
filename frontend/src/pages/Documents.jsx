...
export default function Documents() {
  ...
  const del = async (d) => {
    if (!window.confirm(`Delete "${d.display_name || d.filename}" and all its extracted data?`)) return;
      ...
      {docs.length === 0 ? (
        <Rise delay={0.1}>
          <div className="mt-10 flex flex-col items-center rounded-2xl border border-dashed border-seamdark bg-white px-6 py-16 text-center">
            ...
            <Link to="/pipeline"
                  className="mt-5 flex items-center gap-2 rounded-lg bg-coal px-4 py-2.5 text-sm font-semibold text-white transition-all hover:-translate-y-px hover:bg-amber-500">
        ...
        <Rise delay={0.1}>
          <div className="mt-6 grid grid-cols-1 gap-4 sm:grid-cols-2 xl:grid-cols-3">
            {docs.map((d) => {
              ...
              return (
                <article key={d.id}
                         className="group relative flex flex-col rounded-xl border border-seam bg-white p-5 shadow-card transition-all duration-300 hover:-translate-y-0.5 hover:border-coal hover:shadow-lift">
                  ...
                  <div className="mt-3 flex flex-wrap gap-1.5">
                    <span className="rounded-full border border-coalline bg-coalsoft px-2 py-0.5 font-mono text-[10px] uppercase text-coal">{d.content_norm || d.doc_type}</span>
                    {d.subsidiary && <span className="rounded-full border border-seam bg-paper px-2 py-0.5 text-[10.5px] text-stone-600">{d.subsidiary}</span>}
                    {d.doc_date_raw && <span className="rounded-full border border-seam bg-paper px-2 py-0.5 font-mono text-[10.5px] text-stone-600">{d.doc_date_raw}</span>}
                  </div>

                  {(kwMap[d.id] || []).length > 0 && (
                    <div className="mt-2.5 flex flex-wrap gap-1">
                      {kwMap[d.id].slice(0, 3).map((kw) => (
                        <Link key={kw} to={`/search?tag=${encodeURIComponent(kw)}`}
                              className="rounded-full border border-seam bg-paper px-2 py-0.5 text-[10.5px] text-stone-500 transition-colors hover:border-coalline hover:text-coal">#{kw}</Link>
                      ))}
                    </div>
                  )}

                  <div className="mt-auto flex items-center justify-between gap-2 border-t border-seam pt-3">
                    <div className="flex items-center gap-3 font-mono text-[10.5px] text-stone-400">
                      {!!d.page_count && <span className="flex items-center gap-1" title="pages"><Files className="h-3 w-3" />{d.page_count}</span>}
                      {!!d.ocr_pages && <span title="OCR pages">OCR {d.ocr_pages}</span>}
                      {d.version_group_id && !d.is_current_version
                        ? <span className="rounded-full border border-seam bg-paper px-2 py-0.5 line-through">superseded</span>
                        : d.status === "failed"
                          ? <span className="rounded-full border border-red-200 bg-red-50 px-2 py-0.5 text-red-700">failed</span>
                          : null}
                    </div>
                    <div className="flex items-center gap-1 opacity-0 transition-opacity duration-200 group-hover:opacity-100 max-md:opacity-100">
                      <Link to={`/doc/${d.id}`} title="Open"
                            className="rounded-md p-1.5 text-stone-400 transition-colors hover:bg-coalsoft hover:text-coal"><ExternalLink className="h-4 w-4" /></Link>
                      <button type="button" title="Delete" onClick={() => del(d)}
                              className="rounded-md p-1.5 text-stone-400 transition-colors hover:bg-red-50 hover:text-red-700"><Trash2 className="h-4 w-4" /></button>
                    </div>
                  </div>
                </article>
              );
