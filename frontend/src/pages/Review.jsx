        ...
        <Rise delay={0.08}>
          <div className="mt-4 rounded-xl border border-coalline bg-coalsoft/60 p-4 text-[13.5px]">
            ...
            <div className="mt-2 grid grid-cols-2 gap-2 sm:grid-cols-5">
              <div><span className="font-mono text-[17px] font-semibold">{stats.documents_searched}</span><span className="ml-1.5 text-stone-500">docs searched</span></div>
              <div><span className="font-mono text-[17px] font-semibold">{stats.relevant_passages}</span><span className="ml-1.5 text-stone-500">passages</span></div>
              <div><span className="font-mono text-[17px] font-semibold">{stats.facts_used}</span><span className="ml-1.5 text-stone-500">facts used</span></div>
              <div><span className="font-mono text-[17px] font-semibold">{stats.conflicts}</span><span className="ml-1.5 text-stone-500">conflicts</span></div>
      ...
      <Rise delay={0.16}>
        <div className="mt-6 flex flex-wrap items-center gap-3">
          <button onClick={() => act("approved")} disabled={busy}
                  className="flex items-center gap-2 rounded-lg bg-emerald-600 px-5 py-2.5 text-sm font-semibold text-white transition-all hover:-translate-y-px hover:shadow-lift disabled:opacity-50">
            <Check className="h-4 w-4" /> Approve Report
          </button>
          <div className="flex min-w-[280px] flex-1 flex-wrap items-center gap-2">
            <input value={note} onChange={(e) => setNote(e.target.value)} placeholder="Reason for returning..." required
                   className="min-w-[200px] flex-1 rounded-lg border border-seamdark bg-white px-3 py-2.5 text-sm focus:border-coal focus:outline-none" />
            <button onClick={() => act("returned")} disabled={busy || !note.trim()}
                    className="rounded-lg border border-seam px-4 py-2.5 text-sm font-semibold text-stone-600 transition-colors hover:border-red-300 hover:text-red-700 disabled:opacity-50">
              Return for Revision
            </button>
          </div>
