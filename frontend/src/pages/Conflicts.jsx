...
function ConflictCard({ c, onStatus }) {
  ...
  return (
    <article className="rounded-xl border border-seam bg-white p-5 shadow-card">
      ...
      <div className="mt-4 rounded-lg border border-coalline bg-coalsoft/60 px-3 py-2.5 text-[12.5px]">
        <p><strong className="text-coal">Why flagged:</strong> values differ by {c.spread_pct}% on the same key.</p>
      ...
      <div className="mt-4 flex flex-wrap items-center gap-2">
        {["open", "acknowledged", "resolved"].map((s) => (
          <button key={s} disabled={busy} onClick={() => setStatus(s)}
                  className={`rounded-lg border px-3 py-1.5 text-[12.5px] font-semibold capitalize transition-colors ${c.status === s ? "border-coal bg-coal text-white" : "border-seam text-stone-500 hover:border-coal hover:text-coal"}`}>
      ...
      <Rise delay={0.05}>
        <div className="mt-6 space-y-4">
          {list === null ? (
            ...
            list.map((c, i) => <ConflictCard key={c.key} c={c} onStatus={load} />)
          )}
        </div>
      </Rise>
    </div>
