...
a summary. Usage: python -m cmpdi_intel.scripts.ingest <file-or-dir> [...]"""

import sys
from pathlib import Path

from cmpdi_intel import config, db
from cmpdi_intel.pipeline import pipeline


def main(argv):
    if not argv:
        print("Usage: python -m cmpdi_intel.scripts.ingest <file-or-dir> [...]")
        return 2
    paths = []
    for a in argv:
        p = Path(a)
